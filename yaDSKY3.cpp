 ...
 * Filename:	uDSKY.cpp
 * Contact:	Steve Botts
 * Date:   	07/31/2017 (C) added uDSKY features only by s.botts [sb]

VERS 07/26/17
Added all keys to HotkeyEvent() Not sure why the K, C, R, and P keys were missing.  P key 
Still does not work.  Cleaned up the code, removed a bunch of printf(), added the '*' on uDSKY
feature to return the control of flashing to uDSKY hardware. This elimited the lag in display.
TODO: Add command line control for Serial config.  
TODO: make full (non graphic) controller UI. 
	(hopefully one that i can keep in control focus for RPI standalone operation) 
TODO Fix PRO key functional in hardware keys and uDSKY. 

WORKING: VERS 7/24/2017
First working version. Made it yaDSKY3 which is derived from yaDSKY2 by RSB. 
Currently, it is only a hack to demonstrate the hardware. I added the I/O stream 
and configuraton for a serial port.  

Ultimately, I would like to entirely replace the yaDSKY GUI with a stand alone program 
that can control yaAGC.  Also, i did not block the yaDSKY2 funcitons in any way, which turns
out to be real handy as the criteria for starting communications is not fully understood
and using the graphic keys are still required.  The keyboard seems to have some of the same
issues as well so i don't think this is fault of the hardware which is a keyboard as well.
Because of the delay in transmitting the commands to usbDSKY hardware, the display lags 
a bit. Flashing is not as smooth in next rev should give control back to DSKY to flash 
VERB, NOUN, OPR ERR, and KEY REL displays.

INITIAL EDITS VERS 7/20/2017:  Added test code to demonstrate sending commands to USB miniDSKY (uDSKY) 
The serial command structure is fairly simple to send values to the various displays.
Pxx<cr><lf> writes xx to PGM ( also lower case 'p' works
Vxxf<cr><lf> writes xx to VERB, f= '*' indicates flashing, ' ' for no flash ( also lower case 'v' works)
Nxxf<cr><lf> writes xx to NOUN, f ='*' indicates flashing, ' ' for no flash ( also lower case 'n' works)
Rxsyyyyy<cr><lf> writes 5 yyyyy digits to Rx ( x = 1, 2, or 3) s = Sign = '+', '-', or ' ' 

Indicator Lamps: 
Lamps 11 - 17    Cxy<cr><lf> ( x = 1 - 7) y =1 (ON), y = 0 (OFF) y=0 '*' will flash the indicator.
Lamps 21 - 27    Wxy<cr><lf> ( x = 1 - 7) y =1 (ON), y = 0 (OFF) y=0 '*' will flash the indicator.
Ax,<cr><lf> ( COMP ACTY x =1 (ON), x = 0 (OFF) )
Default serial port is: "/dev/ttyACM0" found by looking for Teensey in /dev on the Rpi. 


 * Made From:	yaDSKY2.cpp
 ...
 *              2017-07-20 SB   This is an experimental version to control 
 *                              hardware version of DSKY on USB serial port
 *                              the current implemetation is a Teensey 3.2
 *                              with AGC running on raspian rpi 3.0 This is 
 *				the first "working" version.  This is basically 
 *				a hack into yaDSKY2 to communicate with my
 *				USB DSKY hardware.  
 *				TODO: 	Flashing is not smooth,
 *				TODO: 	uDSKY Keys are intermittent, still need 
 *					GUI,they don't start up immeadiately.
 *				TODO:	Replace yaDSKY with a stand alone program. 
 *
...
#include <unistd.h>		//Used for usbDSKY
#include <fcntl.h>		//Used for usbDSKY
#include <termios.h>		//Used for usbDSKY
using namespace std;
...
//Added Globals [sb]
static char buffer[20];		//Used for usbDSKY
static int fd_uDSKY   = -1;	//Used for usbDSKY
...
// 7/26/17 added these events below for full usbDSKY keys function [sb]
    case 'c':
    case 'C': on_ClrButton_pressed (event); break;
    case 'k':
    case 'K': on_KeyRelButton_pressed (event); break;
    case 'r':
    case 'R': on_RsetButton_pressed (event); break;
    case 'p':
    case 'P': on_ProButton_released (event); break;  //released?
...
void MainFrame::set_properties()
{
    ...
    SetTitle(wxT("yaDSKY2 by Ron Burkey and Steve"));
    wxIcon _icon;
...
bool yaDskyApp::OnInit()
{
  ...
  cout << string ("yaDSKY3 Apollo DSKY simulation, ver " VER(NVER) ", built " __DATE__ " " __TIME__ "\n");
  cout << string ("07/20/2017 by Steve Botts\n");
  cout << string ("**NOTE: USB  Serial port is active in this version.  [SB]\n");


printf("Setting up Serial Port\n");

	//----------------------------------------
	//----- SETUP USB DSKY's SERIAL PORT -----
	//----------------------------------------

//	int fd_uDSKY   = -1;
	
	//OPEN the File descriptor pointing to the USB DSKY Serial port


printf("Open the Serial Port\n");

	fd_uDSKY = open("/dev/ttyACM0", O_RDWR | O_NOCTTY | O_NDELAY | O_NONBLOCK);		//Open in non blocking read/write mode


	if (fd_uDSKY == -1)
	{
		//ERROR - CAN'T OPEN SERIAL PORT
		printf("Error - Unable to open SERIAL PORT.  ...dammmit\n");
	}
	...
	tcsetattr(fd_uDSKY, TCSANOW, &options);

	tcflush(fd_uDSKY, TCIFLUSH);
	//----- TX BYTES -----
...
//printf("Sending test C1* to USB DSKY to Flash UPLINK Lamp\n");	
...
//	if (fd_uDSKY != -1)
//	{
//		int count = write(fd_uDSKY, &tx_buffer[0], (p_tx_buffer - &tx_buffer[0]));		//Filestream, bytes to write, number of bytes to write
//		if (count < 0)
//		{
//			printf("Serial port TX error\n");
//		}
//	}
//printf("Serial Port testing is complete...\n");
//printf("UPLINK Lamp should be flashing!\n");

//sprintf(buffer, "W4*\r\n");
//write(fd_uDSKY,buffer , 6);
//printf(buffer);
...
TimerClass::Notify ()
{
  extern int VerbNounFlashing;  //I am putting this back to allow DSKY to control
...
TimerClass::Lamp(int Value, int State)
{
char c = ' ';
  switch (Value)
        {
	...
	case 2:		// STBY
		...
		c = 'C';
		break;
/*  have to do these cases in Acton Incomming IO 
	case 3:		// KEY REL ( on = FLASHING)
		//if( State == 0) sprintf(buffer, "C4%d\r\n", State);
		//else if ( State == 1) sprintf(buffer, "C4*\r\n");
		sprintf(buffer, "C4%d\r\n", State);
		write(fd_uDSKY,buffer , 5);
		printf(buffer);
		c = 'C';
		break;
	case 4:		// OPR ERR ( on = FLASHING)
		sprintf(buffer, "C5%d\r\n", State);
		write(fd_uDSKY,buffer , 5);
		c = 'C';
		break;
*/
	...
	case 13:		// VEL
		...
		c = 'W';
		break;
	default:		//Not a valid lamp - do nothing
		sprintf(buffer, "lamp%d= %d\r\n", Value, State);
	//	write(fd_uDSKY,buffer , 8);
	//	printf(buffer);
		return;
	
	}
//done -  this is a test to shorten this method...
//	if (Value >6) Value -= 6;
...
//May need to add the flag to do flashing on the DSKY
//#define DSKY_KEY_REL  000020 = 16
//#define DSKY_FLASH  000040 = 32
//#define DSKY_OPR_ERR  000100 = 64
//

if (Channel == 011){
	//Do flashing KEL REL
 	if((Value&16) != (Last11&16)){	//no change - to do it once 
   		if (!(Value & 16)) sprintf(buffer, "C40\r\n");
   		if (Value & 16) sprintf(buffer, "C4*\r\n");
	write(fd_uDSKY,buffer , 5);
	printf(buffer);
	}
	// Do flashing OPR ERR
 	if((Value&64) != (Last11&64)){	//no change - to do it once 
   		if (!(Value & 64)) sprintf(buffer, "C50\r\n");
   		if (Value & 64) sprintf(buffer, "C5*\r\n");
	write(fd_uDSKY,buffer , 5);
	printf(buffer);
	}
	// Do flashing NOUN /VERBS
 	if((Value&32) != (Last11&32)){	//no change - to do it once 
   		if (!(Value & 32)){
			VN_F = ' ';
 			sprintf(buffer, "V%c%c%c\r\n", V[1], V[0], VN_F);  //no flash
			write(fd_uDSKY,buffer , 5);
			printf(buffer);
 			sprintf(buffer, "N%c%c%c\r\n", N[1], N[0], VN_F);
			write(fd_uDSKY,buffer , 5);
			printf(buffer);
		}
   		if (Value & 32){ 
			VN_F = '*';
			sprintf(buffer, "V%c%c%c\r\n", V[1], V[0], VN_F); //flashing!
			write(fd_uDSKY,buffer , 6);
			printf(buffer);
			sprintf(buffer, "N%c%c%c\r\n", N[1], N[0], VN_F);
			write(fd_uDSKY,buffer , 6);
...
//Last11 = Value;  //save to see if bits change next time.
}
...
//flashing of OPR ERR  on the hardware..[sb]
...
//flashing of KEY REL on the hardware..[sb]
...
//***Format USB output for Master Mode  (PROG) [sb]
P[0]=Rly2INt(Value & 0x1F); //Right
P[1]=Rly2INt((Value >> 5) & 0x1F); //Left


sprintf(buffer, "P%c%c\r\n", P[1], P[0]);
//printf("P[%c][%c]\n", P[1], P[0]);
write(fd_uDSKY,buffer , 5);
...
//***Format USB output for Verbs  (VERB) [sb]
V[0]=Rly2INt(Value & 0x1F); //Right
V[1]=Rly2INt((Value >> 5) & 0x1F); //Left
//printf("V[%c][%c]\n", V[1], V[0]);
//sprintf(buffer, "V%c%c\r\n", V[1], V[0]);
sprintf(buffer, "V%c%c%c\r\n", V[1], V[0], VN_F); //
write(fd_uDSKY,buffer , 6);
...
//***Format USB output for Nouns  (NOUN) [sb]
N[0]=Rly2INt(Value & 0x1F); //Right
N[1]=Rly2INt((Value >> 5) & 0x1F); //Left
//printf("N[%c][%c]\n", N[1], N[0]);
sprintf(buffer, "N%c%c%c\r\n", N[1], N[0], VN_F); //
write(fd_uDSKY,buffer , 6);
...
//***Format USB output for Register (R1) [sb]
R[1][1] = Rly2INt(Value & 0x1F); //Right

sprintf(buffer, "R1%c%c%c%c%c%c\r\n", R[1][0],R[1][1],R[1][2],R[1][3],R[1][4],R[1][5]);
write(fd_uDSKY,buffer ,10);
 ...
 //***Format USB output for Register (R1) [sb]
	  ...
	  // R1 SIGN + has priority if both bits are set
	  if (0 != (RSign & 2)) R[1][0]= '+';	
	  else if (0 != (RSign & 1))	R[1][0] = '-';	
	  else	R[1][0] = ' ';
 
//***Format USB output for Register (R1) [sb]
R[1][2] = Rly2INt((Value >> 5) & 0x1F); //Left
R[1][3] = Rly2INt(Value & 0x1F); //Right
...
R[1][4] = Rly2INt((Value >> 5) & 0x1F); //Left
R[1][5] = Rly2INt(Value & 0x1F); //Right
...
write(fd_uDSKY,buffer , 10);
 ...
 //***Format USB output for Register (R2) [sb]
	  // R2 SIGN + has priority if both bits are set
	  if (0 != (RSign & 2)) R[2][0]= '+';	
	  else if (0 != (RSign & 1))	R[2][0] = '-';	
	  else	R[2][0] = ' '; 

R[2][1] = Rly2INt((Value >> 5) & 0x1F); //Left
R[2][2] = Rly2INt(Value & 0x1F); //Right


sprintf(buffer, "R2%c%c%c%c%c%c\r\n", R[2][0],R[2][1],R[2][2],R[2][3],R[2][4],R[2][5]);
write(fd_uDSKY,buffer , 10);
...
//printf(buffer);
	...
	case 0x2000:	// AAAA=4
	  ...
	  if (0 != (Value & 0x0400))
	    ...
	    R2Sign &= ~1;
	  RSign = R2Sign;    
 //***Format USB output for Registers (R2) [sb]
 // R2 SIGN + has priority if both bits are set
...
R[2][3] = Rly2INt((Value >> 5) & 0x1F); //Left
R[2][4] = Rly2INt(Value & 0x1F); //Right
...
//printf(buffer);
	...
	case 0x1800:	// AAAA=3
 //***Format USB output for Register (R2, R3) [sb]
R[2][5] = Rly2INt((Value >> 5) & 0x1F); //Left
...
R[3][1] = Rly2INt(Value & 0x1F); //Right

sprintf(buffer, "R3%c%c%c%c%c%c\r\n", R[3][0],R[3][1],R[3][2],R[3][3],R[3][4],R[3][5]);
write(fd_uDSKY,buffer , 10);
...
//printf(buffer);
 ...
 //***Format USB output for Register (R3) [sb]
// R3 SIGN + has priority if both bits are set

	  if (0 != (RSign & 2)) R[3][0]= '+';	
	  else if (0 != (RSign & 1))	R[3][0] = '-';	
	  else	R[3][0] = ' ';
R[3][2] = Rly2INt((Value >> 5) & 0x1F); //Left
R[3][3] = Rly2INt(Value & 0x1F); //Right
...
write(fd_uDSKY,buffer , 10);
	...
	case 0x0800:	// AAAA=1
	  ...
	  if (0 != (Value & 0x0400))
	    ...
	    R3Sign &= ~1;
	  RSign = R3Sign;  
 //***Format USB output for Registers (R3) [sb]
// R3 SIGN + has priority if both bits are set

	  if (0 != (RSign & 2)) R[3][0]= '+';	
	  else if (0 != (RSign & 1))	R[3][0] = '-';	
	  else	R[3][0] = ' ';
R[3][4] = Rly2INt((Value >> 5) & 0x1F); //Left
R[3][5] = Rly2INt(Value & 0x1F); //Right
...
//Let's not turn the usb DSKY  VN displays off..
//sprintf(buffer, "V  \r\n");
//write(fd_uDSKY,buffer , 5);
//printf(buffer);
//sprintf(buffer, "N  \r\n");
...
//Let's put the FLASH Flag '*'  on VN instead...
//Now doing it in channel11
//sprintf(buffer, "V%c%c*\r\n", V[1], V[0]);
//write(fd_uDSKY,buffer , 6);
//printf(buffer);
//sprintf(buffer, "N%c%c*\r\n", N[1], N[0]);
//write(fd_uDSKY,buffer , 6);
//printf(buffer);
        }
    }
